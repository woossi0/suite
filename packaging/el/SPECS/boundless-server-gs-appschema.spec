...
Name: boundless-server-gs-appschema
Version: REPLACE_VERSION
Release: REPLACE_RELEASE
Summary: Boundless Server App Schema Plugin
Group: Applications/Engineering
License: GPLv2
URL: http://geoserver.org
BuildRoot: %{_WORKSPACE}/boundless-server-gs-appschema/BUILDROOT
Requires(post): bash
Requires(preun): bash
Requires:  unzip, boundless-server-geoserver >= CURRENT_VER, boundless-server-geoserver < NEXT_VER
Obsoletes: suite-gs-appschema
Conflicts: suite-gs-appschema
AutoReqProv: no
...
%description
App Schema plugin for Boundless Server.

%prep

%install
mkdir -p %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
cp -p %{_WORKSPACE}/SRC/BoundlessServer-ext/appschema/*.jar %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
