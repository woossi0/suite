...
Name: boundless-server-gs-db2
Version: REPLACE_VERSION
Release: REPLACE_RELEASE
Summary: Boundless Server DB2 Plugin
Group: Applications/Engineering
License: GPLv2
URL: http://geoserver.org
BuildRoot: %{_WORKSPACE}/boundless-server-gs-db2/BUILDROOT
Requires(post): bash
Requires(preun): bash
Requires:  unzip, boundless-server-geoserver >= CURRENT_VER, boundless-server-geoserver < NEXT_VER
Obsoletes: suite-gs-db2
Conflicts: suite-gs-db2
AutoReqProv: no
...
%description
DB2 plugin for Boundless Server.

%prep

%install
mkdir -p %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
cp -p %{_WORKSPACE}/SRC/BoundlessServer-ext/db2/*.jar %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
