...
Name: boundless-server-gs-oracle
Version: REPLACE_VERSION
Release: REPLACE_RELEASE
Summary: Boundless Server Oracle Plugin
Group: Applications/Engineering
License: GPLv2
URL: http://geoserver.org
BuildRoot: %{_WORKSPACE}/boundless-server-gs-oracle/BUILDROOT
Requires(post): bash
Requires(preun): bash
Requires:  unzip, boundless-server-geoserver >= CURRENT_VER, boundless-server-geoserver < NEXT_VER
Obsoletes: suite-gs-oracle
Conflicts: suite-gs-oracle
AutoReqProv: no
...
%description
Oracle plugin for Boundless Server.

%prep

%install
mkdir -p %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
cp -p %{_WORKSPACE}/SRC/BoundlessServer-ext/oracle/*.jar %{buildroot}/opt/boundless/server/geoserver/WEB-INF/lib
