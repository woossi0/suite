...
The Layer Importer differs from the standard `GeoServer interface for loading data <../../geoserver/data/>`_ in a few ways. The Layer Importer loads stores and publishes layers in a single step, populating the fields with intelligent defaults. It also operates on multiple files at once. Finally, the Layer Importer creates a unique style (:term:`SLD`) for every layer loaded.
