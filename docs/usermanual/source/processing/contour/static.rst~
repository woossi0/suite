...
Using the GeoServer WPS process **gs:Contour** it is possible to generate a vector contour layer of a given raster layer. This process can be accessed through the **WPS Request Builder** by going to :guilabel:`Demos` and selecting :guilabel:`WPS Request Builder`. Once there, select :guilabel:`gs:Contour` in the box marked :guilabel:`Choose process`. Enter the following information for your specific data:
...
Too many lines of equal width can make them hard to distinguish, so the next step is to add a rule that will display every fifth band (500 meters) as thicker (1px instead of 0.4px). Determining if the value of the band is a multiple of 500 is accomplished with the ``IEEERemainder`` filter function.
...
Once that is done, associate this style with the original layer. This is done by going to the :guilabel:`Layers` menu, clicking on the layer name, clicking on the :guilabel:`Publishing` tab, and selecting the style from the drop-down menu called :guilabel:`Default style`. Click :guilabel:`Save` when done.
