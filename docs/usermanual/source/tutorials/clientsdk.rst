...
By default, the application will be published on port 8080.  To run the server on another port, provide the :command:`-p <port>` option to the debug command::

    suite-sdk debug -p 9080 path/to/myapp

This will make your application available for debugging at ``http://localhost:9080/``.
...
This will make your remote GeoServer available locally at ``http://localhost:8080/geoserver`` .

Note that the :command:`-p` and :command:`g` options can be used together.  For example, you could debug your application on port 9080 while accessing a local GeoServer running on port 8080::

    suite-sdk debug -p 9080 -g http://localhost:8080/geoserver path/to/myapp 

Again, this would make your application available at ``http://localhost:9080/`` while making GeoServer available on the same port at ``http://localhost:9080/geoserver``.
