...
.. literalinclude:: artifact/heatmap_example.ysld
   :language: yaml
   :linenos:
   :emphasize-lines: 6-11,14-17,19-23
...
* On **lines 9-11**, **outputBBOX**, **outputWidth**, and **outputHeight** define the output parameters, which are obtained from internal environment variables set during rendering, as described above.

The **raster** symbolizer (starting on **line 14**) is used to style the raster generated by the transformation.

* On **line 15**, **opacity** specifies an overall opacity of 0.6 for the rendered layer.
* On **line 16**, **color-map** defines a color map with which to symbolize the output raster. The color map uses a **type** of ``ramp`` (**line 17**), which produces a smooth transition between colors. The five **entries** (**lines 19-23**) between 0.0 and 0.02 are displayed with a fully transparent color of white, which makes areas where there no influence from data points invisible.
