...
rmdir /s /q %buildroot%\postgis_upgrade
...
copy %POSTGIS_UTILS%\postgis_restore.pl %buildroot%\postgis_upgrade\
...
copy ..\..\installer\common\pgupgrade\postgis_upgrade.pl %buildroot%\postgis_upgrade\
copy postgis_upgrade.exe %buildroot%\postgis_upgrade\
echo "building postgis_upgrade.exe"

cd %buildroot%\postgis_upgrade
zip postgis_upgrade-3.0.zip *
echo "Zipping"

scp -P 7777 -i C:\hudson\.ssh\installer_upload_key -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no postgis_upgrade-3.0.zip jetty@astromech.opengeo.org:/var/www/winbuilds
del postgis_upgrade-3.0.zip
